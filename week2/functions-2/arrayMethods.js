...
// CODE HERE
const totalPopulation = populations.reduce((acc, crvl) => acc + crvl);
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter((val) => val.CP > 200);
...
const total = orders.map((el) => (el.price += el.price * el.tax));
...
const purchaseSum = (purchases) =>
  purchases
    .filter((el) => el.owner === "Bob")
    .map((obj) => obj["price"])
    .reduce((acc, crvl) => acc + crvl);

console.log(purchaseSum(purchases));
