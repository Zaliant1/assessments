...
const gettingNames = (array, callback) => {
  callback(array[0]);
};
...
uniq(names, (result) => {
  result = `The new names array with all the duplicate items removed is ${[
    names,
  ]}.`;
});
...
const each = (array, callback) => {
  array.forEach((el, ind) => callback(el, [ind]));
};
